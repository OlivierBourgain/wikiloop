...
public class Loop {
     ...
     *
     * Pour économiser un peu de mémoire, au lieu de stocker les données dans une
     * Map<String, String> (Page -> Sucesseur), on utilise un tableau de String
     * pour stocker les pages, et un tableau d'entiers qui pour chaque page
     * donne l'index du successeur.
     *
     * Le tableau de page est trié, et on retrouve la bonne page par une recherche
     * binaire.
     */
    private final String[] pages = new String[2450000];
    private final int[] firstLinks = new int[2450000];
    private int cpt;
    ...
    private int load(File f) throws IOException {
        ...
        while (it.hasNext()) {
            if (cpt % 100000 == 0) log.debug("1 - Chargement ligne " + cpt);
            String s = it.next();
            String[] t = s.split("#");
            // On ignore les id.
            pages[cpt] = t[1];
        ...
        while (it.hasNext()) {
            if (cpt2 % 100000 == 0) log.debug("2 - Chargement ligne " + cpt2);
            String s = it.next();
            String[] t = s.split("#");
            int page = getIdx(t[1]);
            int next = getIdx(t[3]);
    ...
    private String get(String s) {
        int idx = getIdx(s);
        if (!pages[idx].equals(s)) return null;
    ...
    public List<Page> loop(String root) {
        ...
        while (tortue != lievre) {
            if (lievre == null || get(lievre) == null) return res;
            tortue = get(tortue);
